...
def apply_cleanup(test_code):
    test_code = cleanup1(test_code)

    return test_code

# Remove non code string generated by Starcoder2 with prompt_2
def cleanup1(test_code):
...
def apply_heuristics(test_code, prompt):
    test_code = remove_after_patterns(test_code)
    test_code = keep_code_in_backticks(test_code)
    test_code = remove_prompt_from_generated_test(test_code, prompt)
